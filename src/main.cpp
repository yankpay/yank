...
#if defined(NDEBUG)
# error "Yank cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in lank) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nBits, int nHeight, const CAmount& nFees)
{
    ...
    int64_t nSubsidy = 0;
    if((nHeight >= 11160 && dDiff > 75) || nHeight >= 22320) {
        // 2222222/(((x+2600)/9)^2)
